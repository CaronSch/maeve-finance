...
import { CircularGauge } from "~~/components/maeve/CircularGauge";
import { Skeleton } from "~~/components/maeve/Skeleton";
import { useScaffoldReadContract } from "~~/hooks/scaffold-eth/useScaffoldReadContract";
import { MaeveTokenConfig, formatBpsAsPercent, useMaeveTokens } from "~~/utils/maeve";
      ...
      <header>
        ...
        <h1 className="text-3xl font-light">
          ...
          <span className="group relative inline-flex align-middle ml-1">
            <InformationCircleIcon className="w-5 h-5 text-base-content/30 hover:text-base-content/60 cursor-help" />
            <span
              className="invisible group-hover:visible opacity-0 group-hover:opacity-100 absolute left-1/2 -translate-x-1/2 top-full mt-2 z-10 w-72 text-xs font-normal leading-relaxed rounded-lg p-3 pointer-events-none"
              style={{
            ...
            >
              This score reflects how many lenders accept this token as collateral and at what LTV. Higher = more
              trusted.
            </span>
          </span>
        </h1>
  ...
  return (
    ...
    >
      ...
      <div className="col-span-12 md:col-span-5 flex items-center justify-center md:justify-start">
        <CircularGauge scoreBps={Number(scoreBps)} size={88} strokeWidth={7} label="LTV" />
      </div>
